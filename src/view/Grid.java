...
import controller.Commandcontrol;
import model.Board;
...
public class Grid {
    ...
    public static final int ROWS = 20;//the columns  of the grid
    public static final int COLS = 10;//the row of the grid
    public static final int DIM = 4;

    Block[][] mBlock;//having all the pieces from block class
...
//        paints board with green blocks
        for (int i = 0; i < ROWS; i++) {
            for (int j = 0; j < COLS; j++) {
                mBlock[i][j] = new Block(false, Color.green, i, j);
            }
        }
    }
...
//                game ends and clear board
                Commandcontrol.getInstance().setPlaying(false);
                Commandcontrol.getInstance().setGameOver(true);
                clearGrid();
            }
...
//            completed row if the size is equal to the number of columns
            if (fullRowItems.size() == Grid.COLS) {
                while (fullRowItems.size() > 0) {
                    ...
                    Commandcontrol.getInstance().addScore(blck.getPoints());
